...
>
    <div
            ...
            x-data="visualBuilder({
                ...
                widgetPreviews: @js($widgetPreviews ?? []),
            })"
    ...
    @script
    <script>
        Alpine.data('visualBuilder', (config) => ({
            state: config.state,
            ...
            getWidgetPreview(widget) {
                ...
                switch (type) {
                    ...
                    case 'image':
                        const imageValue = data.src ?? data.image ?? data.url ?? '';
                        ...
                        const imageLabel = typeof imageValue === 'number'
                            ? `media #${imageValue}`
                            : String(imageValue).split('/').filter(Boolean).pop() || 'image';

                        return `🖼 ${imgAlt || imageLabel}`;
                ...
                if (data.src || data.image || data.url) {
                    return '🖼 ' + (data.src || data.image || data.url);
                }
                return '(empty)';
            },
