...
<head>
    @php
        $context = app(\App\Support\ErrorPageContext::class)->resolve403(
            request(),
...
<body class="min-h-screen bg-stone-950 text-stone-50 antialiased">
    <main class="relative isolate flex min-h-screen items-center overflow-hidden">
        <div class="absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(220,38,38,0.20),_transparent_34%),radial-gradient(circle_at_bottom_right,_rgba(120,113,108,0.20),_transparent_30%),linear-gradient(180deg,#0c0a09_0%,#1c1917_100%)]"></div>
        <div class="absolute inset-x-0 top-0 h-px bg-white/10"></div>
        <div class="relative mx-auto w-full max-w-5xl px-6 py-16 lg:py-24">
            <div class="max-w-3xl">
                <p class="text-xs font-semibold uppercase tracking-[0.45em] text-red-300">{{ $eyebrow }}</p>
                ...
                <div class="mt-8 flex flex-wrap gap-3">
                    <a href="{{ $primaryUrl }}" class="inline-flex items-center rounded-full bg-red-400 px-5 py-3 text-sm font-medium text-stone-950 transition hover:bg-red-300">
                        {{ $primaryLabel }}
                    </a>
            ...
            <div class="mt-12 grid gap-4 lg:grid-cols-3">
                @foreach ($highlights as $highlight)
                    <section class="rounded-3xl border border-white/10 bg-white/5 p-5 shadow-2xl shadow-black/10 backdrop-blur">
                        <p class="text-sm font-semibold text-red-300">{{ $highlight['title'] }}</p>
