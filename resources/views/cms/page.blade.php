...
@section('content')
    <article class="mx-auto max-w-4xl px-6 py-16">
        @unless ($page->is_homepage)
            <p class="text-sm font-semibold uppercase tracking-[0.3em] text-amber-600">{{ $settings->site_tagline ?? 'Page' }}</p>
        ...
        @if (filled($page->content))
            <x-page-builder :blocks="$page->content" :page="$page" class="mt-10" />
        @endif
    </article>
@endsection
